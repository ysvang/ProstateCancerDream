#' validationTablePreprocess
#' 
#' Performs various data cleanups and prepares validation table for modeling
#' 
#' Author: Yeeleng Scott Vang (ysvang@uci.edu)
#================================================================================================================
# This script reads in the validation table csv file and applies preprocessing to obtain a useable validation design matrix.
...
# read in data file 
validationTable <- read.csv("final\\CoreTable_validation.csv")


# create duplicate dataframe from validationTable for modification
tDF <- validationTable
# delete covariates to match training design matrix
tDF$DOMAIN <- NULL
tDF$STUDYID <- NULL
tDF$RPT <- NULL
tDF$LKADT_P <- NULL
...
tDF$HEAD_AND_NECK <- NULL # delete covariate to match training design matrix
...
# ECOG_C, replace '.' with most frequent value '1', then one-hot encode
tDF$ECOG_C <- factor(tDF$ECOG_C)
tDF$ECOG_C[tDF$ECOG_C == '.'] <- '1'  # replace '.' with '1'


# HGTBLCAT one hot encoding
tDF$HGTBLCAT[tDF$HGTBLCAT == '>=200-220'] <- '>=180-200'  # since only one patient has this value, replace with '>=180-200'
tDF$HGTBLCAT[tDF$HGTBLCAT == ''] <- '>=160-180'  # replace blanks with most frequent '>=160-180'
...
# WGTBLCAT one hot encoding
tDF$WGTBLCAT[tDF$WGTBLCAT == ''] <- '>=70-80'  # replace blanks with most frequent '>=70-80'
tDF$WGTBLCAT<- factor(tDF$WGTBLCAT)
levels(tDF$WGTBLCAT) <- c(levels(tDF$WGTBLCAT), ">=130-140")  # add this category missing from validation set but in leaderboard set
tDF$WGTBLCAT = factor(tDF$WGTBLCAT, levels(tDF$WGTBLCAT)[c(1:3,11,4:10)])
...
row.names(tDF) <- NULL # row.names column isn't actually counted in number of variables count (still 121)
#tDF[is.na(tDF)] <- 0  # replace any remaining 'na' cell with 0

tDF
write.csv(tDF, file="testDesignMatrix.csv", row.names=FALSE)
